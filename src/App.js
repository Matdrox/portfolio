import { useState } from 'react';
import Menu from './components/Menu';
import Themes from './components/buttons/Themes';

function App() {
  const [theme, setTheme] = useState(
    'bg-zinc-100 transition ease-in-out duration-500'
  );
  const [primaryColor, setPrimaryColor] = useState(
    'bg-white transition ease-in-out duration-500 h-32 w-32 rounded shadow-lg'
  );

  return (
    <div className={`transition ease-in-out duration-500 ${theme}`}>
      <Themes
        changeTheme={(theme) => setTheme(theme)}
        changePrimary={(primaryColor) => setPrimaryColor(primaryColor)}
      />
      <div className='bg-white p-6 max-w-sm mx-auto rounded-xl shadow-lg flex items-center space-x-4'>
        <div>
          <div className='text-xl font-medium text-black'>ChitChat</div>
          <p className='text-slate-500'>You have a new message!</p>
        </div>
      </div>
      <Menu changePrimary={primaryColor} />
    </div>
  );
}

export default App;
